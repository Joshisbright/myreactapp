        ...
        <nav className="hidden md:flex space-x-8">
          {navLinks.map((link) => (
            <div key={link.name} className="relative group">
              <Link href={link.href}>
                <span
                  className={`nav-link font-poppins text-sm tracking-wider font-medium cursor-pointer ${
            ...
            <div className="flex flex-col items-center space-y-10">
              {navLinks.map((link, index) => (
                ...
                >
                  <Link href={link.href}>
                    <span
                      className={`text-3xl font-poppins font-medium tracking-wider cursor-pointer transition-all duration-300 ${
                        location === link.href
